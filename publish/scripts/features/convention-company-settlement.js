(function initConventionCompanySettlement(root) {
  ...
  function incidentFor(event, prep, completed) {
    const crowding = Math.max(0, prep.promotion - prep.safety);
    const chance = clamp(8 + crowding * 0.7 + (4 - completed) * 9
      + Math.max(0, 50 - prep.safety) * 0.35, 3, 65);
    const roll = hash(`${event.id}:${prep.quality}:${prep.safety}:${prep.promotion}`) % 100;
  ...
  function preview(state, event) {
    ...
    const forecastProfit = grossRevenue - emergencyCost - prep.budget - Game.conventionCompany.bidCost;
    const payout = grossRevenue - incidentCost - emergencyCost;
    ...
    return {
      ...
      emergencyCost, payout, prepBudget: prep.budget, forecastProfit, projectProfit, reputationDelta,
