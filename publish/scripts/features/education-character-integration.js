(function initEducationCharacterIntegration(root) {
  ...
  function act(state, type) {
    ...
    Game.characterAttributes.gain(state, '智力', gains[type] || 0.5, `学习安排:${type}`);
  ...
  function exam(state, label, subjects) {
    ...
    state.stats.智力 = Game.characterAttributes.playerValue(state, '智力');
    try {
  ...
  function monthly(state) {
    ...
    if (!['home', 'graduate', 'workforce'].includes(state.education.schoolStage)) {
      Game.characterAttributes.gain(state, '智力', 0.22, '持续教育');
    }
  }
  ...
  Game.educationSystem = Object.freeze({ ...base, act, exam, monthly, readiness });
}(window));
